...
% firstinits=false,
maxcitenames=2,
date=year, maxbibnames=99,url=false]{biblatex}
...
\section{Case study: The Rhône River at Beaucaire}
    ...
    \caption{Distributions used for the different sources of stage errors (in meters). $\mathcal{N}$ and $\exp$ represent Gaussian and Exponential distributions respectively.}
    \label{tab:StageErr}
    \end{table}
 ...
 \section{Results}
    ...
    % Static parameters $a$ and $c$ (figure \ref{subfig:a's and c's Pt}). The $a$ parameters are precisely estimated seeing the narrow posterior distributions. However, $c$ posterior distributions are as wide as priors. This is due to the fact that $c$ priors are already very precise because they come from simplified Manning-Strickler formula for which the exponent is exactly $5/3$.
